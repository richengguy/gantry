...
@click.option(
    ...
    help=("An account user name.  Required for Basic authentication."),
    type=str,
)
@click.option(
    "--pass",
    ...
    help=("An account password.  Required for Basic authentication."),
)
@click.option(
    "--cert",
    "-c",
...
def cmd_authenticate(
    ...
    if not basic_auth and not api_auth:
        result = Prompt.ask(
            "[grey30]>[/grey30] Authentication Type", choices=["basic", "token"]
        )
    ...
    try:
        ...
        raise CliException(
            "Authentication failed...run with 'gantry -d' to see traceback."
        )
...
def cmd_version(opts: ProgramOptions) -> None:
    ...
    try:
        ...
        _logger.exception("%s", str(e), exc_info=e)
        raise CliException(
            "Failed to get version...run with 'gantry -d' to see traceback."
        )
...
@click.option(
    "--description", "-d", help="An optional description for the new repo.", type=str
)
@click.argument("name")
@click.pass_obj
def cmd_repos_create(opts: ProgramOptions, description: str | None, name: str) -> None:
    ...
    try:
        ...
        _logger.exception("%s", str(e), exc_info=e)
        raise CliException(
            "Failed to create repo...run with 'gantry -d' to see traceback."
        )
...
def cmd_repos_delete(opts: ProgramOptions, name: str) -> None:
    ...
    try:
        ...
        _logger.exception("%s", str(e), exc_info=e)
        raise CliException(
            "Failed to delete repo...run with 'gantry -d' to see traceback."
        )
...
def cmd_repos_list(opts: ProgramOptions) -> None:
    ...
    try:
        ...
        _logger.exception("%s", str(e), exc_info=e)
        raise CliException(
            "Failed to list repos...run with 'gantry -d' to see traceback."
        )
