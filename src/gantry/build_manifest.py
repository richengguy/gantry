...
class DockerComposeEntry(Entry):
    ...
    def __post_init__(self) -> None:
        if self.compose_file.name != "docker-compose.yml":
            raise BuildManifestBadFilePathError(
                "compose-file", "docker-compose.yml", self.compose_file.name
            )
...
class ImageEntry(Entry):
    ...
    def __post_init__(self) -> None:
        if self.source.name != "Dockerfile":
            raise BuildManifestBadFilePathError(
                "source", "Dockerfile", self.source.name
            )
...
class BuildManifest:
    ...
    def load(path: PathLike) -> "BuildManifest":
        ...
        manifest = BuildManifest(
            parsed["name"], description=parsed.get("description"), source=path
        )
        item: dict
            ...
            match item["type"]:
                case EntryType.DOCKER_COMPOSE:
                    manifest.append_entry(
                        DockerComposeEntry(
                            Path(item["compose-file"]), item.get("is-deployable", True)
                        )
                    )
                case EntryType.IMAGE:
                    manifest.append_entry(
                        ImageEntry(item["image"], Path(item["source"]))
                    )
                case _:
