...
class ActivityDisplay(ABC):
    ...
    def __init__(
        self, logger: logging.Logger, *, process_name: str | None = None
    ) -> None:
        """
        Parameters
        ----------
    ...
    def stop(self, *, stop_surface: bool = True, clear_surface: bool = True) -> None:
        ...
        if clear_surface:
            ...
            self._surface.update(
                self._create_display_group(console_output), refresh=True
            )
...
class ProcessDisplay:
    ...
    def start(self) -> "ProcessDisplay":
        ...
        self._task = self._progress.add_task(
            "", start=True, total=None, **self._task_fields
        )
        self._started = True
        return self
...
if __name__ == "__main__":
    ...
    multi_activity_display = MultiActivityDisplay(
        x, logger, description="Running Multi-stages", stage_fn=stage_fn
    )
